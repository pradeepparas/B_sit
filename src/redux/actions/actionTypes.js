//  Login Actions
export const AUTH_START = 'AUTH_START';
export const AUTH_SUCCESS = 'AUTH_SUCCESS';
export const AUTH_FAIL = 'AUTH_FAIL';
export const AUTH_LOGOUT = 'AUTH_LOGOUT';
export const LOGIN_SUCCESSFULL = "LOGIN_SUCCESSFULL";

// Station-Management Actions
export const ADD_STATION = 'ADD_STATION';
export const EDIT_STATION = "EDIT_STATION";
export const DELETE_STATION = "DELETE_STATION";
export const SET_ISADD = "SET_ISADD";
export const FETCH_CONTRACTORS = "FETCH_CONTRACTORS";
export const STATION_ERROR = "STATION_ERROR";
export const STATION_SUCCESS = "STATION_SUCCESS";
export const IS_SUBMITTED = "IS_SUBMITTED";
export const FETCH_STATIONS = "FETCH_STATIONS";
export const FETCH_STATION_BYPARAMS = "FETCH_STATION_BYPARAMS";
export const SET_ISLOADING = "SET_ISLOADING";

// User-Management Actions
export const FETCH_USER_BYPARAMS = "FETCH_USER_BYPARAMS";
export const ADD_USER = 'ADD_USER';
export const DELETE_USER = "DELETE_USER";
export const EDIT_USER = "EDIT_USER";
export const SET_ROLES = "SET_ROLES";
export const FETCH_ROLES_BYPARAMS = "FETCH_ROLES_BYPARAMS";
// export const SET_ISADD = "SET_ISADD";

// Vendors
export const FETCH_VENDOR_BYPARAMS = "FETCH_VENDOR_BYPARAMS";
export const FETCH_VENDORS = "FETCH_VENDORS";
export const FETCH_VENDOR_MANAGEMENT_BYPARAMS = "FETCH_VENDOR_MANAGEMENT_BYPARAMS";
export const FETCH_VENDOR_MANAGEMENT = "FETCH_VENDOR_MANAGEMENT";
// DashBoard
export const SET_DASHBOARD_COUNT = "SET_DASHBOARD_COUNT";

// SFMIS Services
export const FETCH_CATEGORY_SERVICES_BYPARAMS = "FETCH_CATEGORY_SERVICES_BYPARAMS";
export const FETCH_CATEGORY_SERVICES = "FETCH_CATEGORY_SERVICES";
export const FETCH_SFMIS_SERVICES_BYPARAMS = "FETCH_SFMIS_SERVICES_BYPARAMS";
export const FETCH_ITEMS_BYPARAMS = "FETCH_ITEMS_BYPARAMS";

// Feedback
export const FETCH_FEEDBACK_BYPARAMS = "FETCH_FEEDBACK_BYPARAMS";
